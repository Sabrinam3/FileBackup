...
private:
	...
	*/
	class File
	{
		//CONSTRUCTOR
	public:
		File(path p, ul s, bool st) : path(p), size(s), status(st) {}
...
//PRIVATE METHODS - ACCESSED FROM INSIDE THE FILEMANAGER CLASS
	...
	*/
			...
			if (is_regular_file(recursiveIt->status()))
			{
				...
				if (extension == ".c" || extension == ".cpp" || extension == ".h" || extension == ".hpp")
				{
					...
					if (!fileExists)
					{
						...
						File addFile(recursiveIt->path(), fileSize, false);
	...
	ul totalSize_ = 0;
	//A shared_ptr to a vector that will hold the files found in the source directory. 
	//You might be wondering why I decided to use a smart pointer instead of just a vector - I kind of just wanted to experiment with using a shared_ptr for one, and for two I thought
	//if a directory had alot of files, it would cause alot of overhead to create a distinct vector for every file manager object. I thought about changing it to a vector, but this works so why not?
